...
void button(int window_width, const char* btnText, Rectangle button, bool hover){
	...
	DrawText(btnText,
			button.x + button.width/2 - textW/2,
			button.y + button.height/2 - btnFontSize/2,
			btnFontSize,
			RAYWHITE);
}
...
void menu(int window_width, Rectangle startButton, Rectangle playersButton, bool hoverStart, bool hoverPlayers)
{
	...
	button(window_width, "START", startButton, hoverStart);

	button(window_width, "2PLAYERS", playersButton, hoverPlayers);
